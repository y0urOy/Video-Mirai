...
class WanDiffusionWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        return_hidden_at_layer: Optional[int] = None,  # Foresight-Forcing: extract hidden state at this layer
    ) -> torch.Tensor:
        ...
        if kv_cache is not None:
            output = self.model(
                ...
                return_hidden_at_layer=return_hidden_at_layer
            )
            
            if return_hidden_at_layer is not None:
                flow_pred, hidden_state = output
            else:
                flow_pred = output
            ...
            if clean_x is not None:
                ...
                if return_hidden_at_layer is not None and isinstance(output, tuple):
                    flow_pred, hidden_state = output
                    flow_pred = flow_pred.permute(0, 2, 1, 3, 4)
                else:
                    flow_pred = output.permute(0, 2, 1, 3, 4)
            else:
        ...
        if flow_pred is None and return_hidden_at_layer is not None and hidden_state is not None:
            # Early return from model: only hidden state available, skip x0 conversion
            return None, None, hidden_state
        ...
        if return_hidden_at_layer is not None and hidden_state is not None:
            return flow_pred, pred_x0, hidden_state
