...
class ODERegressionLMDBDataset(Dataset):
    def __init__(self, data_path: str, max_pair: int = int(1e8),
                 teacher_features_path: str = None):
    ...
    def __getitem__(self, idx):
        ...
        Outputs:
            ...
            - teacher_hidden (optional): Tensor of shape (num_frames, teacher_dim). Mean-pooled hidden state from teacher model.
        """
        ...
        if self.teacher_features_path is not None:
            feat_path = os.path.join(self.teacher_features_path, f"{idx:05d}.pt")
            teacher_hidden = torch.load(feat_path, map_location="cpu").float()
            result["teacher_hidden"] = teacher_hidden
        return result
